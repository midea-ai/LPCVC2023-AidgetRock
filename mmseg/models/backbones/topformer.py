"""Modified from https://github.com/hustvl/TopFormer/blob/main/mmseg/models/backbones/topformer.py
TopFormer: Token Pyramid Transformer for Mobile Semantic Segmentation
Zhang 2022 CVPR (Apache License 2.0)(MIT License)"""
...
class TokenPyramidModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.layers):
            if i!=8: #modified: add a new line
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        # token * N 
        for i in range(3): #modified: from 'range(self.block_num)' to 'range(3)'
...
#         H = (H - 1) // self.stride + 1
#         W = (W - 1) // self.stride + 1
#         return torch.cat([F.adaptive_avg_pool2d(inp, (W, H)) for inp in inputs], dim=1)

#Modified 'PyramidPoolAgg' as following:
class PyramidPoolAgg(nn.Module):
    def __init__(self, stride):
        super().__init__()
        self.stride = stride
    ...
    def forward(self, inputs):
        return torch.cat([F.avg_pool2d(inputs[i],kernel_size=self.cfg[i],stride=self.cfg[i],padding=self.pad[i],ceil_mode=False,count_include_pad=False) for i in range(4)], dim=1)
...
class InjectionMultiSum(nn.Module):
    ...
    def forward(self, x_l, x_g):
        ...
        x_l: local features
        '''
        #B, C, H, W = x_l.shape #原始代码
        local_feat = self.local_embedding(x_l)
        
        global_act = self.global_act(x_g)
        sig_act = F.interpolate(self.act(global_act), size=(self.size, self.size), mode='bilinear', align_corners=False)
        
        global_feat = self.global_embedding(x_g)
        global_feat = F.interpolate(global_feat, size=(self.size, self.size), mode='bilinear', align_corners=False)
...
class Topformer(BaseModule):
    ...
    def forward(self, x):
        x= F.interpolate(x, (288,288), None, 'bilinear', False) #modified: add a new line to resize input.
        ouputs = self.tpm(x)
        out = self.ppa(ouputs)
        out = self.trans(out)
